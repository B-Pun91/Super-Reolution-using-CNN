...
def Disp_Image(img, title):
      ...
      plt.title(title)
      plt.show()
      print("Size of the", title, "image:", img.shape)
...
weights = {
    'w1': tf.Variable(tf.random_normal([9, 9, 1, 64], stddev=1e-3), name='w1'),# 9*9 is the patch size, 1 = number of input channels, 64 is the number of outputs from the first conv layer
...
#First CNN layer
output_conv1 = sess.run(conv1, feed_dict={inputs: input_})
...
# Displaying Blurred Image and printing PSNR
Disp_Image(blurred_image, 'Blurred')
B = skimage.measure.compare_psnr(groudtruth_image.reshape(255, 255), blurred_image.reshape(255, 255), data_range=1)
print('PSNR of Blurred Image:', B)

# Displaying SRCNN Output image  and printing PSNR
Disp_Image(output_.reshape(243, 243), 'SRCNN')
A = skimage.measure.compare_psnr(groudtruth_image[6:249, 6:249], output_.reshape(243, 243), data_range=1)
print('PSNR of SR Image:', A)
